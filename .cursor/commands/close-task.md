...
4. **Если после удаления подраздел (например `### Theming System`) не содержит ни одной задачи** — удали весь подраздел целиком. Не оставляй плейсхолдеры вроде «(Нет активных задач — X завершена)».

### Шаг 7. Подготовить Memory Bank

**Режим A** — задача из "Current Task":

- Перенеси задачу в "Last Completed Task" (task_id, название, дата, статус COMPLETED, ссылка на completed-запись).
...
- **backlog.md:** Не оставлять пустые подразделы с плейсхолдерами. Удалять подраздел целиком, если в нём не осталось задач.
...
- [ ] Артефакты (архив, рефлексия) найдены
...
- [ ] Пустые подразделы backlog удалены
