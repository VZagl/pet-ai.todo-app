...
🔄 **IN PROGRESS** — BUILD: Фазы 1 и 2 выполнены, остались тесты/сборка и коммиты
...
- [ ] BUILD: запуск тестов и сборки локально, коммиты (2 коммита по плану)
...
- [ ] Запустить `pnpm test` и `pnpm build` (локально; в среде — EPERM esbuild)
- [ ] Коммит (например: refactor(naming): обычные файлы в kebab-case)

### Фаза 3: Финализация

- [ ] Финальный прогон тестов и сборки (локально)
